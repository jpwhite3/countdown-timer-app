 ...
 *     crosses 60s and again at 30s (only when `flash` is enabled).
 *   - `critical` is true when ≤10s remain, and stays true through overtime.
 *
 * Audio (only when `audio` is enabled, requires a prior user gesture
 * via `unlockAudio()` for browsers to honor playback):
 *   - Warning tone at 60s and 30s thresholds.
 *   - Per-second tick during the last 10s (>0).
 *   - Done tone exactly when the countdown reaches zero.
 */
export function useTimerCues(countdown, { flash, audio }) {
  ...
  useEffect(() => {
    ...
    for (const threshold of FLASH_THRESHOLDS_MS) {
      if (prev > threshold && total <= threshold && total > 0) {
        if (!firedThresholdsRef.current.has(threshold)) {
          firedThresholdsRef.current.add(threshold)
          if (flash) {
            setFlashing(true)
            window.setTimeout(() => setFlashing(false), FLASH_DURATION_MS)
          }
          if (audio) Cues.warning()
        }
      }
    }

    if (total <= CRITICAL_THRESHOLD_MS && total > 0 && audio) {
